...
func (a *MultiLLMAdapter) Generate(ctx context.Context, prompt string, model string) (<-chan LLMToken, error) {
	ch := make(chan LLMToken, 100)
	...
	go func() {
		...
		var client llm.StreamingLLMClient
		for providerName, c := range a.providers {
			if providerName != "" && effectiveModel != "" && providerName+"/" == effectiveModel[:len(providerName)+1] {
				client = c
				break
			}
		}

		if client == nil && len(a.providers) > 0 {
		...
		if client == nil {
			ch <- LLMToken{Token: "[Error: no LLM client configured]", Done: true}
			return
		}
		...
		resp, err := client.GenerateStream(ctx, req, func(accumulated string) {
			ch <- LLMToken{Token: accumulated}
		})

		if err != nil {
			ch <- LLMToken{Token: "[Error: " + err.Error() + "]", Done: true}
			return
		}

		if resp != "" {
			ch <- LLMToken{Token: resp}
		}
